...
- **Auto-updater**: Tauri v2 updater plugin — on launch, silently checks `latest.json` at GitHub Releases. If a newer version exists, shows native dialog "Install Now / Later". If accepted, opens browser to the direct installer download URL so the user can run the new installer themselves.
- **Current version**: v1.1.1 (in `src-tauri/tauri.conf.json`)
- **Signing keypair**: Ed25519 (minisign format, no password). Public key in `tauri.conf.json`. Private key in GitHub secret `TAURI_SIGNING_PRIVATE_KEY`. Generates `.sig` files at build time; required for update verification.
- **Publishing a new version**: Create tag via GitHub API → Actions builds `.msi` + `-setup.exe` + signed `.sig` + `latest.json` → auto-published to GitHub Releases → existing users notified on next launch
- **latest.json**: Generated by PowerShell step in workflow; contains version, date, download URL, and base64 signature. Required for auto-updater.
