...
- **Auto-updater**: Tauri v2 updater plugin — on launch, silently checks `latest.json` at GitHub Releases. If a newer version exists, shows native dialog "Install Now / Later". If accepted, downloads, installs, and restarts automatically (true in-app update, no browser needed).
- **Current version**: v1.2.0 (in `src-tauri/tauri.conf.json`)
